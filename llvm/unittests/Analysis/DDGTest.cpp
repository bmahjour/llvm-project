      ...
      *M, "foo",
      [&](Function &F, LoopInfo &LI, DependenceInfo &DI, ScalarEvolution &SE) {
        ...
        for (DDGNode *N : DDG) {
          ...
          if (llvm::any_of(IList, [](Instruction *I) { return I->getName() == "loadASubI"; })) {
            LoadASubI = N;
            break;
          }
        }
        ...
        for (DDGNode *N : DDG) {
          ...
          for (DDGNode *M : cast<PiBlockDDGNode>(N)->getNodes()) {
            SmallVector<Instruction *, 8> IList;
            M->collectInstructions([](const Instruction *I) { return true; }, IList);
            if (llvm::any_of(IList, [](Instruction *I) {return I->getName() == "loadBSubI"; })) {
        ...
        assert(PiBlockWithBSubI && "Did not find pi-block containing load of B[i]");
        ...
        for (DDGNode *N : DDG) {
          ...
          SmallVector<Instruction *, 8> IList;
          N->collectInstructions([](const Instruction *I) { return true; }, IList);
          if (llvm::any_of(IList, [](Instruction *I) {return I->getName() == "ff.0"; })) {
            FFPhi = N;
            break;
          }
        }
        ...
        unsigned NumMemoryEdges = llvm::count_if(EL, [](DDGEdge *Edge) { return Edge->isMemoryDependence(); });
        ...
        NumMemoryEdges = llvm::count_if(EL, [](DDGEdge *Edge) { return Edge->isDefUse(); });
        EXPECT_EQ(NumMemoryEdges, 1ull);
      });
}
